...
for (const spriteName in layersBySpriteName) {
  ...
  for (const multiplier of [1, 2]) {
    ...
    for (const [key, layerName] of Object.entries(layers)) {
      ...
      compositeOptions.push({
        input: resolve(tmpDir, `layer-thumbnails-${multiplier}x/${layerName}.png`),
    ...
    const outputPath = resolve(distDir, `${spriteName}-${multiplier}x.jpg`);
  ...
  await writeFile(
    resolve(distDir, `${spriteName}-manifest.json`),
