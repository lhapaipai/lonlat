...
export function getHashString(baseLayer: string, viewState: ViewState) {
...
export function parseHashString(): { baseLayer: string; viewState: ViewState } | null {
  const hash = window.location.hash.replace("#", "");
  if (hash === "") {
    return null;
  }
  const [baseLayer, zoomStr, lngStr, latStr, bearingStr, pitchStr] = hash.split("/");
