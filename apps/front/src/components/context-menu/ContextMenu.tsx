...
import { useEventCallback, useRefDebounce } from "pentatrion-design/hooks";

type CustomContextEvent = CustomEvent<{
  emulated: boolean;
  originalEvent: Event;
}>;
...
export function ContextMenu({
  ...
  const { refs, floatingStyles, context } = useFloating({
     ...
     */
    placement: window.matchMedia("(min-width: 700px)").matches
      ? "right-start"
      : undefined,
    strategy: "fixed",
  ...
  const onContextMenuStable = useEventCallback(function onContextMenu(
    ...
    const originalEvent =
      e instanceof MouseEvent ? e : (e.detail.originalEvent as MouseEvent);
  ...
  useEffect(() => {
    ...
    stableTarget.addEventListener(
      eventName as "contextmenu",
      onContextMenuStable,
    );
    return () => {
      // console.log("remove contextmenu", targetRef);
      stableTarget.removeEventListener(
        eventName as "contextmenu",
        onContextMenuStable,
      );
    };
        ...
        <FloatingFocusManager context={context} initialFocus={refs.floating}>
          ...
          >
            <Dialog
              ...
              className="z-context-menu max-h-80 overflow-auto"
            >
              {Children.map(
                children,
                (child, index) =>
