...
type Options<T = number> = {
  yTolerance: number;
  y: (data: T[], idx: number) => number;
  result: (data: T[], idx: number) => number;
  length: (data: T[]) => number;
  nonextrema: boolean;
};

type LocalExtramaReturn = {
  extrema: [number, number][];
  maxima: [number, number][];
  minima: [number, number][];
  nonextrema?: [number, number][];
  plateaus: [number, number][];
};

type Range = [number, number];

export function localExtrema<T>(
  data: T[],
  partialOptions: Partial<Options<T>>
): LocalExtramaReturn {
  ...
  const options: Options<T> = Object.assign(defaultOptions, partialOptions);
  ...
  const result = (idx: number) => options.result(data, idx);
  const y = (idx: number) => options.y(data, idx);
  const lastIndex = dataLength - 1;
  let lastSlope; // -1: falling, +1: rising
  const extrema: Range[] = [];
  const maxima: Range[] = [];
  const minima: Range[] = [];
  const plateaus: Range[] = [];
  let lastMin = y(0);
  let lastMax = y(0);
  let thisY = y(1);
  ...
  let i = 2; // next point
  let plateauLength = 0;
  const addMaximum = (a: number, b: number) => (
    maxima.push([a, b]), extrema.push([a, b])
  );
  const addMinimum = (a: number, b: number) => (
    minima.push([a, b]), extrema.push([a, b])
  );
  const addPlateau = (a: number, b: number) => (
    plateaus.push([a, b]), extrema.push([a, b])
  );
  ...
  if (options.nonextrema) {
    ...
    if (extrema.length == 0) {
      ...
      for (let i = 0; i < extrema.length - 1; i++) {
        nonextrema.push([extrema[i][1], extrema[i + 1][0]]);
      }
