...
Principal component. All others components described in this library need to be defined as children of this component.
This component take his props from `MapOptions`, map events, and some extra configuration options.

It instantiates a Map object adds event listeners and is responsive to only a few props (see reference below).

## Hello world.

The `RMap` component accepts the `style` and `mapStyle` prop to respectively define the style attribute of the containing element `div.maplibregl-map` and the style to apply to the Map instance.

import HelloWorld from "./HelloWorld.tsx";
import helloWorldHtml from "./HelloWorld.tsx?shiki";

<TwoCols first={<ShikiContent html={helloWorldHtml} />} second={<HelloWorld />} />

Internally your style prop is wrap inside a `useMemo`, you can therefore define this object online without affecting performance
...
## `MapOptions` / `events` and reactivity

You can use all options from `MapOptions` as props for the `RMap` component. Some of these options are responsive and some are not. To distinguish them, non-reactive options have been prefixed with `initial`

import Options from "./Options.tsx";
import optionsHtml from "./Options.tsx?shiki";

<TwoCols first={<Options />} second={<ShikiContent html={optionsHtml} />} foreground="second"  />


If your prop is an array or an object define your variable
- outside your component if it's a readonly value
- inside a `useMemo` to prevent unnecessary renderings
