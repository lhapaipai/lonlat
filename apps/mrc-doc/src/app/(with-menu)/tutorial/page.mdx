...
Welcome to the tutorial! We'll be building a small, but feature-rich map app that will help you understand the fundamentals of this library.

## Setup

We'll be using [Vite](https://vitejs.dev/guide/#scaffolding-your-first-vite-project) for our dev server / bundler for this tutorial.
...
npm i maplibre-theme # only for this tutorial to have a nice theme
...
function App() {
  return (
    <RMap
      ...
      mapStyle="https://openmaptiles.geo.data.gouv.fr/styles/osm-bright/style.json"
    >
      <RMarker longitude={mountain.lng} latitude={mountain.lat} />
    </RMap>
  );
}
...
```tsx {2,4-5,7,29,31-32}#add {9}#delete
import { MapLayerMouseEvent } from "maplibre-gl";
import "maplibre-theme/dist/modern.css";

// needed for the RGradientMap component
...
import layerSwitcherHtml from "./LayerSwitcher.extract.tsx?shiki";
import LayerSwitcher from "./LayerSwitcher.tsx";

<TwoCols first={<ShikiContent html={layerSwitcherHtml} className="windowed-code" />} second={<LayerSwitcher />} />
