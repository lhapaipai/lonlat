...
import liftingCode from "./50-lifting.extract.tsx?code={2,4-5,8,15,27-31,33-38}%23add%20{1,10}%23delete";
...
Note: `RGradientMarker` is a React wrapper of a `GradientMarker` class who extends `Marker` from
`maplibre-gl`. When the marker is added to the map it is integrated into a DOM node which is not managed
...
import layerSwitcherCode from "./70-layer-switcher.extract.tsx?code={3,6-9,13-15,18-21,28,30-33}";
